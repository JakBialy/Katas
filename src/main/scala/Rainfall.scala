object Rainfall extends App {
  ...
  def prepareData(data: String): Map[String, List[Double]] = {
    data.split("\n").map { line =>
      val cityData = line.split(":").toList

      cityData match {
        case city :: data :: Nil =>
          val pattern = """[-+]?[0-9]*\.?[0-9]+""".r
          val convertedNo = pattern.findAllIn(data).toList.map(_.toDouble)
          Map(city -> convertedNo)
        case _ => throw new Exception("Wrong data!")
      }
  ...
  def mean(town: String, strng: String): Double = {
    prepareData(strng).get(town).map {
      cityData =>
        cityData.sum / cityData.size
    }.getOrElse(-1.0)
  }

  def variance(town: String, strng: String): Double = {
    prepareData(strng).get(town).map { cityData =>
      val mean = cityData.sum / cityData.size
      cityData.map { v =>
        Math.pow(v - mean, 2)
      }.sum / cityData.size
    }.getOrElse(-1.0)
